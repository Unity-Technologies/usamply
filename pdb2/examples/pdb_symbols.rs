...
fn dump_pdb(filename: &str) -> pdb::Result<()> {
    ...
    println!("{:?}", di);
    let x = pdb.extra_streams()?;
    println!("{:?}", x);

    for sm in di.section_map()?.iterator() {
        println!("{:?}", sm.unwrap());
    }

    for sc in di.section_contributions()?.iterator() {
        println!("{:?}", sc.unwrap());
    }
    ...
    println!("Address map: {:?}", address_map);
    pdb.sections()?.unwrap().iter().for_each(|&section| {
